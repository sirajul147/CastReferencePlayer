...
sampleplayer.CastPlayer = function (element) {
    ...
    /**
     * The original onSetVolume callback
     * @private {?function(!cast.receiver.MediaManager.Event)}
     */
...
sampleplayer.CastPlayer.prototype.initMessageBus = function () {
    ...
    this.messageBus.onMessage = function(event) {
        ...
        switch (message.requestCode) {
            ...
            case self.MSG_CODE.REQUEST_SET_ACTIVE_TRACKS: {
                console.log('Diagnal Msg', message);
                if('selectedAudioLang' in message) {
                    if(message.selectedAudioLang) {
                        self.changeAudioTrack(null, message.selectedAudioLang);
                    }
                }
...
sampleplayer.CastPlayer.prototype.queueNextEpisode_ =
    function (message) {
        ...
        fetch(media.customData.baseURL + 'media/videos/' + media.customData.mediaId).then(function (res) {
            ...
            fetch(media.customData.baseURL + 'media/series/' + series_id + '/seasons/' + season_id + '/episodes/' + episode_id + '/next').then(function (res) {
                return res.json();
            }).then(function (next) {
                ...
                self.checkMediaAccess_(next.media_id, media).then(function (res) {
                    ...
                    fetch(media.customData.baseURL + 'media/videos/' + next.media_id).then(function (next_media) {
                        ...
                        fetch(media.customData.baseURL + 'player/progress/' + next.media_id, {
                            headers: {
                                "Content-Type": "application/json",
                                "XSSESSION": media.customData.sessionToken
                            }
                            ...
                            console.log('Diagnal -> Next Media', next_media, progress);
...
/**
 * Broadast current stream info to all senders
 */
...
this.selectedSubtitleLang = null;
...
/**
 * Called when we receive a SET_VOLUME message.
 *
 * @param {!cast.receiver.MediaManager.Event} event The editTracksInfo event.
 * @private
 */
sampleplayer.CastPlayer.prototype.onSetVolume_ = function (event) {
    console.log('Diagnal Change volume -> ', event);
